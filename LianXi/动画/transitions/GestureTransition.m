...
- (void)doPushAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    PushTestViewController *toVC = (PushTestViewController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    //拿到当前点击的cell的imageView
//    UITableViewCell *cell = [fromVC.collectionView cellForItemAtIndexPath:fromVC.currentIndexPath];
    UIView *containerView = [transitionContext containerView];
    //snapshotViewAfterScreenUpdates 对cell的imageView截图保存成另一个视图用于过渡，并将视图转换到当前控制器的坐标
    UIView *tempView = [fromVC.testView snapshotViewAfterScreenUpdates:NO];
    tempView.frame = [fromVC.testView convertRect:fromVC.testView.bounds toView: containerView];
    //设置动画前的各个控件的状态
    fromVC.testView.hidden = YES;
    toVC.view.alpha = 0;
    fromVC.testView.hidden = YES;
